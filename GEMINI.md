# mycats-pro GEMINI ガイドライン

## 0. 目的と参照

- 本ファイルはルート `AGENTS.md` の全内容を Gemini 系 LLM が安全に消化できるよう再構成したものです。プロジェクト規約そのものは `AGENTS.md` が唯一のソースであり、本書は **Gemini モデルで作業する際の補助レイヤー** です。
...
- 公式ドキュメントより新しい指針を適用する場合は、根拠 URL と最終更新日をこのファイルに追記してください。
...
### 1-2. モデル選定とパラメータ
- コード生成や厳密な推論は `gemini-2.5-pro` or `gemini-3.0-pro`（利用可能な場合）を既定とし、軽量応答は `gemini-2.5-flash` を検討。
...
以下は 2025-11-24 時点の `AGENTS.md` を Gemini 用に複製した全文です。内容差分を作る場合は必ず両ファイルを同期してください。
