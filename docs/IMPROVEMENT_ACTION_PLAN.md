...
- 🟥 **公開条件のボトルネック:** CSRF・ENV・RateLimit（C1/C2/H1）が未着手で、PWA/公開フェーズを完全にブロック中。
...
| 領域 | 状態 | メモ |
| --- | --- | --- |
| セキュリティ Phase1 (C1/C2/H1) | 🟥 Blocked | ここが未完。PWA/本番公開・医療データの正式リリース条件。|
...
| --- | --- | --- | --- | --- | --- |
| C1 | CSRF 保護（サーバー + クライアント + E2E）| 4h | Backend/Frontend | 未着手 | `csurf` 導入、`/csrf-token` GET と `apiClient` 連携、E2E 追加。|
| C2 | ENV バリデーション + シークレット管理 | 8h | DevOps/Backend | 未着手 | `validateEnv()` 作成、`.env.example` 縮小、`scripts/generate-secrets.ts` 追加。|

### 🟠 HIGH（Week48-49 で並行）
| ID | ゴール | 所要時間 | Owner | 状態 | 備考 |
| --- | --- | --- | --- | --- | --- |
| H1 | API レート制限強化（エンドポイント別）| 4h | Backend | 未着手 | C1/C2 完了後すぐ実装。Guard + Decorator で適用。|
| H2 | DB インデックス/N+1 最適化 | 4h | Backend | 未着手 | Pedigree/Cats 検索のクエリチューニング。|
...
| --- | --- | --- | --- |
| CSRF Middleware & `/csrf-token` | `csurf` + cookie 設定、GET エンドポイントとE2E | Backend | Week47 | 
| CSRF Token Client | `frontend/src/lib/api/csrf.ts` と `apiClient` で自動添付 | Frontend | Week47 |
| ENV Validation | `validateEnv()` を ConfigModule 起動前に適用 | Backend | Week47 |
| Secret Generator Script | `scripts/generate-secrets.ts` + README 追記 | DevOps | Week48 |
...
### C1: CSRF 保護
1. `backend/src/main.ts` に `csurf` ミドルウェアを追加（cookie 設定: `httpOnly`, `sameSite=strict`, `secure=prod`）。
2. `/api/v1/csrf-token` GET を追加し、token を JSON 返却。
3. `frontend/src/lib/api/csrf.ts` を新設し、token キャッシュ・再取得ロジックを実装。
4. `apiClient` の POST/PUT/PATCH/DELETE リクエストで `X-CSRF-Token` を自動添付。
5. 403 応答のメッセージを日本語化し、E2E テスト（トークン有/無）を backend/test に追加。

### C2: 環境変数 & シークレット
1. `backend/src/config/validate-env.ts`（仮）に Zod スキーマを定義し、`bootstrap` 前に実行。
2. `.env.example` を最小構成に更新（値を空にし説明コメントのみ）。
3. `scripts/generate-secrets.ts` を追加して 32+ 文字のランダム値を生成。
4. `SETUP_GUIDE.md` と `README.md` に使用方法を追記。
...
## 7. チェックリスト
- [ ] C1: CSRF ミドルウェア + `/csrf-token` + E2E.
- [ ] C1: Front `apiClient` で `X-CSRF-Token` 自動添付。
- [ ] C2: `validateEnv` 実装、`.env.example` 更新、`generate-secrets` 追加。
- [ ] H1: Auth API 用レート制限 + 429 メッセージ整備。
- [ ] H2: Prisma インデックス追加、N+1 解消、EXPLAIN で効果検証。
