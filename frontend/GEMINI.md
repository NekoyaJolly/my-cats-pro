# frontend GEMINI ガイドライン

## 0. 目的と参照

- 本書は `frontend/AGENTS.md` を Gemini 系 LLM で再利用しやすくした複製版です。App Router / React / UI 改修時は必ず `AGENTS.md` と内容を同期してください。
- 参考にした Gemini 公式情報（最終更新 2025-11-18 UTC）:
  - [Gemini API Docs](https://ai.google.dev/gemini-api/docs)
  - [プロンプト設計戦略](https://ai.google.dev/gemini-api/docs/prompting-strategies)
- 新たな Gemini ベストプラクティスを取り込む場合は、根拠 URL と日付を明記し、`AGENTS.md` も合わせて更新してください。

## 1. Gemini でフロントエンド作業を行うときのガイド

1. **段階的思考**: `Plan → Implement → Validate → Format` の 4 ステップを system 指示に含め、UI 変更点、アクセシビリティ、i18n をチェックさせる。
2. **構造化コンテキスト**: `<context>` に対象ページ/コンポーネント、`<constraints>` に「Server Component 優先」「"use client" 理由必須」「日本語 UI」などを列挙すると Gemini 3 でも解釈しやすい。
3. **few-shot**: 成功例として「型を更新→実装→テスト→コマンド共有」という一連の応答テンプレを添付し、同じフォーマットで回答させる。
4. **マルチモーダル**: スクリーンショットや Figma 参照が必要な場合は Files API で画像を渡し、`<media>` タグで参照指示を加える。
5. **パラメータ**: コード生成は `temperature=0.4`, `topK=1`, `topP=0.5` を基本。Gemini 3 を利用する場合は温度 1.0 固定推奨のため、確定性が必要なら `gemini-2.5-pro` を利用。
6. **検証**: 回答前に「Server/Client Component 判定」「Tailwind/Mantine ルール」「品質ゲート」の 3 つを満たしたかを自己チェックするよう system 指示を加える。

---

以下は 2025-11-24 時点の `frontend/AGENTS.md` を複製したものです。差分は必ず両ファイルに反映してください。
